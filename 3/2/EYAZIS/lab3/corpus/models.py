...
class SyntacticAnalysis(Base):
    __tablename__ = "syntactic_analyses"
    ...
    text = relationship("TextDocument", backref="syntactic_analyses")
...
class CorpusDB:
    ...
    def clear_corpus(self) -> tuple:
        ...
        try:
            ...
            session.query(SyntacticAnalysis).delete()
    ...
    def save_syntactic_analysis(self, text_id: int, sentence_index: int, original_sentence: str, result_json: str) -> SyntacticAnalysis:
        session = self.get_session()
        try:
    ...
    def get_syntactic_analysis(self, text_id: int) -> List[SyntacticAnalysis]:
        session = self.get_session()
        try:
            return session.query(SyntacticAnalysis).filter_by(text_id=text_id).order_by(SyntacticAnalysis.sentence_index).all()
        finally:
            session.close()

    def update_syntactic_analysis(self, analysis_id: int, result_json: str) -> Optional[SyntacticAnalysis]:
        session = self.get_session()
        try:
            record = session.query(SyntacticAnalysis).filter_by(id=analysis_id).first()
            if record:
    ...
    def delete_syntactic_analysis(self, analysis_id: int) -> bool:
        session = self.get_session()
        try:
            record = session.query(SyntacticAnalysis).filter_by(id=analysis_id).first()
            if record:
