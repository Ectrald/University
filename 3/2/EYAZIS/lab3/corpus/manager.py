...
class CorpusManager:
    ...
    def analyze_syntax(self, text: str) -> List[Dict[str, Any]]:
        """Синтаксический анализ текста (извлечение деревьев зависимостей)."""
        return self.processor.get_syntactic_tree(text)

    def save_syntactic_analysis(self, text_id: int, sentence_index: int, original_sentence: str, result_json: str):
        return self.db.save_syntactic_analysis(text_id, sentence_index, original_sentence, result_json)

    def get_syntactic_analysis(self, text_id: int):
        return self.db.get_syntactic_analysis(text_id)

    def update_syntactic_analysis(self, analysis_id: int, result_json: str):
        return self.db.update_syntactic_analysis(analysis_id, result_json)

    def delete_syntactic_analysis(self, analysis_id: int):
        return self.db.delete_syntactic_analysis(analysis_id)
