...
def analyze_syntax(text: str = Query("")):
    """Синтаксический анализ текста с визуализацией дерева зависимостей."""
    return controller.analyze_text_syntax(text)


@app.get("/api/syntax/text/{text_id}")
def analyze_text_syntax(text_id: int):
    """Синтаксический анализ текста из корпуса по ID."""
    text_doc = controller.corpus.get_text_by_id(text_id)
    if not text_doc:
        return {"error": "Text not found"}
    return controller.analyze_text_syntax(text_doc.content)
