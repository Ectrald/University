...
# ──────────────────────────────────────────────────────────────────────────────
# Задание 1. Neural Style Transfer
# ──────────────────────────────────────────────────────────────────────────────
...
def run_nst():
    ...
    for ax, (step, img) in zip(axes, snapshots):
        ...
        ax.axis('off')
    plt.tight_layout()
    plt.savefig("nst_progress.png", dpi=150)
    plt.show()
    print("Результат сохранён в nst_progress.png")
...
def run_dcgan():
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            show_grid(fake, f"Эпоха {epoch+1}")
    ...
    ax1.plot(epochs, history['loss_D'], label='loss_D')
    ax1.plot(epochs, history['loss_G'], label='loss_G')
    ax1.set_xlabel('Эпоха')
    ax1.set_ylabel('Loss')
    ax1.set_title('Потери генератора и дискриминатора')
    ax1.legend()

    ax2.plot(epochs, history['D_x'],    label='D(x)')
    ax2.plot(epochs, history['D_G_z'],  label='D(G(z))')
    ...
    plt.tight_layout()
    plt.savefig("dcgan_training.png", dpi=150)
    plt.show()
    ...
    show_grid(final_fake, "Финальные сгенерированные изображения")
    ...
    try:
        ...
        print(f"FID: {fid.compute():.2f}  (n={NUM_FID_SAMPLES})")
    ...
    plt.tight_layout()
    plt.savefig("dcgan_interpolation.png", dpi=150)
    plt.show()
